...
App Engine Deployer can deploy new version but cannot shi traffi c
...
```
# Setup logging agent curl -sSO https://dl.google.com/cloudagents/install-logging-agent.sh
...
**Go for Cloud VPN if** :
You want the network to encrypt traffi c OR
...
...PARTITIONED BY
```
(^) CLUSTER DATE(created_date)BY category
...OPTIONS (
...
SET expiration_timestamp= OPTIONS ( TIMESTAMP "2025-01-01 00:00:00 UTC",
partition_expiration_days= 7
)
```
...
Import aer processing by Cloud Dataflow and Cloud
Dataproc
...
Loading data in bulk is free but streaming data is NOT FREE
Offers Best eff ort de-duplication (when you use insertId)
...
Store : Durably and cost effi ciently store data in a
convenient format
...
BigQuery
Hive, Spark, etc.Cloud
Dataproc
CloudStorage
```
```
Real-time storeCloud
Bigtable
CloudSpanner
ML (prediction)Cloud
Bigtable
...
If content is NOT found in the cache (cache miss), request is forwarded to backend (origin server)Response is sent to user and cached
...
https://yourwebsitecom/my-image/ 1 jpg?v= 2
```
...
**Soware development life cycle (SDLC) - Waterfall**

```
Sow are development in multiple long phases :
Requirements
Design
Implementation
Testing
...
**Soware development life cycle (SDLC) - Spiral**

```
Sow are development in smaller iterations :
Start
Iteration 1
Iteration 2
...
...
**Agile is recommended for most sow are
development** :
...
**Soware development life cycle (SDLC) - Agile**
...
```
Getting Better at " Three Elements of Great Sow are Teams "
...
Install right soware and tools on the provisioned resources
...
machineType: disks: <<MACHINE_TYPE>>
- deviceName: type: PERSISTENT boot
boot: true
autoDelete: true
...
```
availability, latency, performance, effi ciency, change
...
Simulate real world traffi c as closely as possible
Test for spiky traffi c - suddenly increases in traffi c
...
Test with production traffi c before going live
...
environment with V2
**Step 3** : Switch all traffi c from V1 to V2 (and
...
production traffi c before releasing
...
```
Option Details
Deploy & shi all traffic at once gcloud app deploy
Deploy v2 without shiing traffic --no-promote
Shi traffic to V2 all at once gcloud app services set-traffi c s1 --splits V2=1
Gradual Migration Add --migrate option to previous command
A/B testing gcloud app services set-traffi c s1 --splits=v2=.5,v1=.5
```
...
Control traffi c using Ingress (or Service)
...
Control inbound and outbound traffi c
...
Offi ce network - for auditing and management
...
Grant apps access to PCI data only aer it has been sanitized with Cloud Data Loss Prevention
```
...
Based on Business value, Teams, Dependencies, Refactoring effort, Licensing and compliance needs,
...
Cloud Bigtable Place clusters in different zones or regions (Each cluster belongs to a zone and you can
...
Stateful applications are more difficult to scale than Stateless
applications
...
Use private load balancing for internal traffi c
...
As an Architect, you need to **understand the needs of diff erent
...
Includes People, Process and Technology (systems, hardware, and soware)
...
Example: Outsourcing
Example Temporary Offi ce Space
```
...
tooling across different types of infrastructure (AWS/Azure/Google Cloud/On-
...
gcloud eventarc triggers create my-audit-log-trigger --destination-run-service=$SERVICE_NAME --destination-run-region=$REGION
--event-filters= --event-filters="type=google.cloud.audit.log.v1.written""serviceName=storage.googleapis.com"
...
"Effi ciently running, managing & monitoring
...
Increase soware development and release velocity
**Release Engineering**
...
Enable Live Migration to keep your VMs running even when there is soware or hardware
maintenance
...
Treat each app component as a separate trust boundaryCreate separate service account for each component/service with the minimum permissions needed
...
**Control ingress and egress** network traffi c using **firewall rules**
Run advanced security and traffi c inspection tools
...
Predict peak traffi c events and plan for them
...
Spread out traffi c for timed promotions and launches
...
```
Provides SaaS based electronic health record sow are for multi-national
medical offices, hospitals, and insurance providers (Exponential growth):
...
Allows predicting race outcomes but lacks the facility to:Support real-time predictions during races (Requirement)
...
Create a merchandisingrevenuestream
```
...
Cloud Spanner Proposed real–time global leader board
